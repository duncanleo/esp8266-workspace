...
void handleWake() {
  ...
  pms.waitForData(Pmsx003::wakeupTime);
  server.send(200, "application/json", "{\"status\": true, \"message\": \"device woken\"}");
}

void handleSleep() {
  pms.write(Pmsx003::cmdSleep);
  server.send(200, "application/json", "{\"status\": true, \"message\": \"device slept\"}");
}
...
void handleDHT22() {
  ...
  char sbuf[150];
  sprintf(sbuf, "{\"status\": false, \"message\": \"ok\", \"data\": {\"humidity\": %f, \"temp_c\": %f, \"heat_index_c\": %f}}", h, t, dht.computeHeatIndex(t, h, false));
  server.send(200, "application/json", sbuf);
}
